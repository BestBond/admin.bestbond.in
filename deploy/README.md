...
**Quick deploy (on server):** from clone root **`./deploy.sh`**. Set `VITE_API_URL` in `.env.production` or export it. Optional: `WEB_ROOT=/var/www/admin.bestbond.in ./deploy.sh` to rsync `dist/` into a flat docroot; optional `RUN_GIT_PULL=1`.

## One-time VPS setup

1. **DNS** — A record for `admin.bestbond.in` → server IP.

2. **Clone** — `git clone ... /var/www/admin.bestbond.in` then `cd /var/www/admin.bestbond.in && chmod +x deploy.sh && ./deploy.sh` (nginx `root` → `.../dist` unless you use `WEB_ROOT`).
